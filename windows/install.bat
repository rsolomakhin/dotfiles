...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p = '%USERPROFILE%\vimfiles\autoload\plug.vim'; $corrupt = (Test-Path $p) -and ((Get-Content $p -Tail 1) -notmatch 'unlet s:cpo_save'); if (!(Test-Path $p) -or (Get-Item $p).LastWriteTime -lt (Get-Date).AddDays(-1) -or $corrupt) { iwr -useb https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim -OutFile \"$p.tmp\"; Move-Item -Path \"$p.tmp\" -Destination $p -Force }" || echo "Cannot download plug.vim" && exit /b 1
