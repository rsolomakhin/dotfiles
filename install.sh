...
git config --global --get user.email || git config --global user.email "rouslan.solomakhin@gmail.com" || die "Cannot set git user email"
...
git config --global --replace-all color.ui true || die "Cannot enable git colors"
